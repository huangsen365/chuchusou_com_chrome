...
**墨清风格署名链接改为作者的「文章」页**。详细见 [releases/v1.18.8.md](./releases/v1.18.8.md)。

### 🔧 改进

- **墨清风格 ⓘ 署名直达作者文章页**：侧边栏风格选择器里墨清风格的 ⓘ、侧边栏帮助文字中的 @zzqgz7326，链接从作者主页改为 https://x.com/zzqgz7326/articles。这个页面列出作者发布的 X 长文，封面就是墨清风格的版式，点开即可看到实际效果；页面顶部仍显示作者头像与简介。显示文字不变。

### 🛠 技术改动

- `coverPrompts.json` 墨清 `credit.url`、`sidepanel.html`、README 三处署名链接同步更新；冒烟测试的署名链接断言改为新地址。
...
- **速答后点「选A改写」，生成的长文没有标题、没有分节**：第一轮速答提示词规定「选 A 时只把约 100 句正文放进 writing block、不得出现标题」，这条规则留在对话里，压过了改写提示词要求的「# 主标题 + ## 小标题」，结果整篇是一长串单句段落、没有任何标题，「注入X草稿」还会拿 writing block 的标签「详细内容」当标题。现在改写提示词明确要求一个主标题、按主线分 3～6 个小节，并声明本轮压过此前对话里的格式要求；速答提示词的 A 规则也加了例外。已经开着的老对话里再点一次「选A改写」同样生效。
- **后台标签页里的站点速答按钮不重建**：浏览器在后台标签页不触发 requestAnimationFrame，知乎 / X / ChatGPT 页面在后台时被重绘的操作栏等到切回来才补按钮。页面隐藏时改用定时器扫描。
- **右键菜单图标「不支持」的探测结果每次重启都丢失**：图标更新失败时写入的存储键和读取的键不一致，每次后台重启都会重试图标更新并重复报错。改为读写同一个键。

### 🔧 改进

- **提示词正文改为 Markdown 文件**：速答、百问、优化、封面、长文改写五份提示词的正文放在 `prompts/*.md`，直接编辑，不用再转义 JSON 字符串；JSON 只保留引擎、分类、轮换表等元数据。运行时拼出的提示词与之前逐字相同。
- **安装包更小**：去掉从未启用的 popup / 侧边栏备用版、不会被加载的三个 popup 模块和构建遗留文件，安装包从约 720 KB 降到约 490 KB（解压后约 2.4 MB → 1.5 MB）。

### 🛠 技术改动

- 删除 86 个从未上线的 TypeScript 平行实现与 `src/assets-json` 镜像，一份逻辑只保留一个源；原先的双轨对比测试改为针对实际运行代码的 golden 快照测试。
- `background/events.js`（2774 行）按领域拆成 `background/events/*.js`，消息改为分发表登记，代码逐行原样搬移。
- ChatGPT 页面选择器集中到 `modules/chatGptDom.js`；新增 `verify:site-adapters` 守卫各站点选择器只写在各自适配器里。
- 新增 `shared/storageKeys.js` 统一登记全部 `ccs_*` 存储键，新增 `verify:storage-keys` 守卫。
- 新增守卫：SW 各脚本顶层重名检测（揪出 3 处同名函数被静默覆盖，已删除被覆盖的副本）、importScripts 列表外的脚本检测；ESLint 的 SW 全局变量改为从加载列表自动收集。
- 移除未使用的 React 依赖；冒烟测试里两处计时抖动改为轮询等待。
