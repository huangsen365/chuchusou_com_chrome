...
> 20 条回归路径自动化 19 条（真 Chrome 装 build 产物端到端：onMessage 11 个 action、
> onConnect 两个分支（sidepanel-alive 生命周期 + welcome-watcher 推送）、tabs.onUpdated 取词、真实选区捕获、菜单动态标题、双 UI
