 ...
 *
 * 实际构建委托给 shared/menuStructureBuilder.js 的纯函数 build()——
 * popup 自己也用同一个 builder 直接构造（不再过 background message），
 * 这里保留 background 端入口仅服务于历史 sendMessage('getMenuStructure') 兼容路径。
 *
...
async function getPopupMenuStructure() {
  const [unifiedConfig, top100Config, fastqaConfig, optimizeConfig, coverConfig, enginesConfig] = await Promise.all([
    loadUnifiedMenuConfig().catch(() => null),
    loadTopQuestionsConfig().catch(() => null),
    loadFastAnswersConfig().catch(() => null),
    loadOptimizedPromptConfig().catch(() => null),
    loadCoverPromptConfig().catch(() => null),
    loadEnginesConfig().catch(() => null)
  ]);

  // loadUnifiedMenuConfig 同时把 toggle 状态灌进 globalThis.menuToggleConfig，
  // 但 builder 是纯函数，自己重新抽取一遍以避免依赖 background 全局；两边结果等价。
